...
class _CreateAITestScreenState extends State<CreateAITestScreen> {
  ...
  Future<void> _generateTest() async {
    ...
    if (formState == null) {
      _showErrorDialog(
        'Form Not Ready',
        'Please return to the test form and try generating again.',
        showRetry: false,
      );
      return;
    }
    ...
    if (_selectedClass == null ||
      ...
      _showErrorDialog(
        'Missing Details',
        'Please select class, section, and subject before generating a test.',
      );
      return;
    }
    ...
    try {
      ...
      if (!lockAcquired) {
        ...
        if (existingLock != null) {
          ...
          _showActiveLockDialog(existingLock);
          return;
        }

        // If lock check itself failed (for example permissions/network),
        // do not falsely show "already assigned" and do not hard-block save.
        debugPrint(
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Could not verify active assignment lock. Saving test anyway.',
              ),
      ...
      _showErrorDialog(
        'Save Failed',
        'Failed to save test: ${e.toString()}',
        showRetry: false,
      );
    }
  }

  void _showActiveLockDialog(TestAssignmentLock activeLock) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'An active test assignment already exists for this class.',
            ),
            const SizedBox(height: 12),
            _buildLockInfoRow('Teacher', activeLock.assignedByTeacherName),
            _buildLockInfoRow('Class', activeLock.classId),
            _buildLockInfoRow('Section', activeLock.sectionId),
            _buildLockInfoRow(
              'Subject',
              activeLock.subjectName.trim().isEmpty
                  ? (_selectedSubject ?? '-')
                  : activeLock.subjectName,
            ),
            _buildLockInfoRow(
              'Assigned At',
              DateFormat(
                'd MMM, h:mm a',
              ).format(activeLock.assignedAtTimestamp),
            ),
            _buildLockInfoRow(
              'Next Available',
              _formatLockTime(activeLock.nextAvailableTimestamp),
            ),
          ],
        ),
        actions: [
